...
- `schema/validator.ts` — Ajv structural validator for the
  `ProgramResource` JSON Schema (program-resources).
...
- `cd schema; npm run validate:resource -- <path/to/resource.json>` —
  import-workflow CLI that wraps the three above, prints the privacy-safe
...
the import-workflow → android-program-runner handoff contract.
